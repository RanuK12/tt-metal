...
@pytest.mark.parametrize("dtype", [ttnn.bfloat16], ids=["bf16"])
@pytest.mark.parametrize(
    "input_shape, index_shape, dim, layout, input_factory",
    [
        pytest.param(
            (1, 2, 4, 32, 64),
            (1, 2, 4, 32, 32),
...
def test_gather_higher_rank(input_shape, index_shape, dim, layout, input_factory, dtype, device):
    _run_gather(
        input_shape,
        index_shape,
        dim,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # dim=-2 (penultimate, equivalent to H on 4D): exercise negative-dim normalization.
        pytest.param((1, 1, 128, 64), (1, 1, 32, 64), -2, ttnn.TILE_LAYOUT, id="TILE_dim_neg2"),
    ],
)
