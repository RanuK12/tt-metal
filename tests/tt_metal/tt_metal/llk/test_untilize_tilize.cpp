...
void run_single_core_unpack_tilizeA_B_reduce_program(
    ...
    const std::uint32_t num_tiles_out = test_config.num_tiles_c;  // reduce col: 1 output row of tiles
    ...
    const std::uint32_t scaler_tile_size = tt::datum_size(tt::DataFormat::Float16_b) * 32 * 32;
...
static void run_quasar_tilize_untilize_test(
    ...
    std::uint32_t output_single_tile_size = tt::tile_size(output_format);
...
TEST_F(LLKQuasarMeshDeviceSingleCardFixture, QuasarComputePackUntilize) {
    std::vector<vector<std::uint32_t>> test_configs = {{4, 12}, {8, 8}, {40, 14}, {2, 40}};
    std::uint32_t run_count = 0;
    constexpr std::uint32_t max_runs = 1;  // TODO (#38092): Remove when we can run back to back tests on Quasar
...
TEST_F(LLKQuasarMeshDeviceSingleCardFixture, QuasarComputePackUntilizeDst) {
    std::vector<vector<std::uint32_t>> test_configs = {{1, 1}, {4, 12}, {8, 8}, {40, 14}, {2, 40}};
    for (auto& cfg : test_configs) {
        for (bool dst_full_sync_en : {true, false}) {
            for (bool fp32_dest_acc_en : {true, false}) {
                for (tt::DataFormat data_format : {tt::DataFormat::Float16_b, tt::DataFormat::Int16}) {
...
TEST_F(QuasarMeshDeviceSingleCardFixture, QuasarComputeUnpackTilize) {
    std::vector<vector<std::uint32_t>> test_configs = {{1, 4}, {4, 1}, {2, 40}};
    for (auto& cfg : test_configs) {
        for (bool dst_full_sync_en : {true, false}) {
            for (bool fp32_dest_acc_en : {true, false}) {
                for (tt::DataFormat data_format : {tt::DataFormat::Float16_b, tt::DataFormat::Int16}) {
