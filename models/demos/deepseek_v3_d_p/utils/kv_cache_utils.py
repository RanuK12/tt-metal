...
def init_kvpe_cache(
    ...
    # fp8_e4m3 host construction does not survive the mesh-mapper path (it forces TILE), so build the
    # cache as bf16 and typecast on device — typecast keeps the ROW_MAJOR layout and the ND shard spec.
    is_fp8 = dtype == ttnn.fp8_e4m3
    tt_kvpe_cache = ttnn.from_torch(
        torch_kvpe_cache,
        dtype=ttnn.bfloat16 if is_fp8 else dtype,
        device=mesh_device,
        layout=layout,
