...
// rounds-trips BOTH tensors through TILE to dodge the noc_async_*_sharded misread. Retires with #47299.
...
Tensor gather(
    ...
    if (ttnn::detail::needs_rm_irregular_composite(input_tensor, input_index_tensor)) {
        ...
        const auto requested_mc = memory_config.has_value() ? memory_config.value() : input_tensor.memory_config();
        const auto tile_out =
            ttnn::gather(tile_input, dim, tile_index, sparse_grad, l1_interleaved, std::nullopt, sub_core_grids);
        // Return to the originally-requested layout/memory_config.
