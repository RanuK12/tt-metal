...
void bind_unary_operation_with_fast_and_approximate_mode(
    nb::module_& mod,
    const std::string& description = "Performs the {0} operation to :attr:`input_tensor` element-wise.",
    const std::string& math_equation = R"math(\mathrm{{output\_tensor}}_i = {0}(\mathrm{{input\_tensor}}_i))math",
    const std::string& range = "",
    const std::string& supported_dtype = "BFLOAT16",
...
void py_module(nb::module_& mod) {
    ...
    bind_unary_operation_with_fast_and_approximate_mode<"erf", &ttnn::erf>(
        mod,
        R"doc(Computes the Gaussian error function of the :attr:`input_tensor` element-wise.)doc",
    ...
    bind_unary_operation_with_fast_and_approximate_mode<"gelu", &ttnn::gelu>(
        mod,
        R"doc(Computes the Gaussian Error Linear Unit activation function element-wise on the :attr:`input_tensor`.
        ...
        It results in a smoooth, continuous function that is gated between 0 and identity.)doc",
    ...
    bind_unary_operation_with_fast_and_approximate_mode<"log", &ttnn::log>(
        mod,
        R"doc(Performs the natural logarithm (base e) operation on the :attr:`input_tensor` element-wise.)doc",
    ...
    bind_unary_operation_with_fast_and_approximate_mode<"log1p", &ttnn::log1p>(
        mod,
        R"doc(Performs the element-wise natural logarithm operation on one plus the :attr:`input_tensor`.)doc",
